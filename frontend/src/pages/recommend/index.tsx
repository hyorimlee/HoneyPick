...
import {Alert, SafeAreaView, StatusBar, TouchableOpacity, View, Dimensions, StyleSheet} from 'react-native'
...
import {Container, CollectionContainer, SearchBarContainer, ItemPageContainer, ItemContainer, NormalText, BoldText, InfoContainer, ImageContainer, ItemBox} from './styles'
              ...
              {items.map(({ title, itemList }, index) => {
                return (
                  <View key={index} style={styles.viewBox}>
                    ...
                    <ItemContainer>
                      {itemList.map(({ _id, thumbnail, priceAfter, priceBefore, title }, index) => {
                        return (
                          <ItemBox
                          onPress={() => goToItem(_id)}
                          >
                            <ImageContainer 
                              source={thumbnail ? {uri: `${Config.IMAGE_BASE_URL}/w128/${thumbnail}`} : require('../../assets/images/sampleimage1.jpg')}
                              imageStyle={{
                                resizeMode: 'contain',
                                borderRadius: 10,
                              }}
                            />
                            <NormalText>
                              { priceAfter ? priceAfter : priceBefore ? priceBefore : '가격정보 없음'}
                            </NormalText>
                            <NormalText>
